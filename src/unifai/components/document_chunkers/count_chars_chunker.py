...
class CountCharsDocumentChunker(DocumentChunker):
    """
    A document chunker that chunks text based on character count.
        chunk_size: Maximum number of characters in each chunk
        chunk_overlap: Number of characters to overlap between chunks. Can be an int (number of characters) or a float (percentage of chunk size)
    Supports both single and multiple separators with recursive splitting.
    """
    provider = "count_chars_chunker"

    def __init__(
            self,
            chunk_size: int = 420,
            chunk_overlap: int|float = 69,
            tokenizer: Tokenizer|Type[Tokenizer]|Callable[..., int] = StrLenTokenizer,
