...
class CountWordsDocumentChunker(DocumentChunker):
    """
    A document chunker that chunks text based on word count.
        chunk_size: Maximum number of words in each chunk
        chunk_overlap: Number of words to overlap between chunks. Can be an int (number of words) or a float (percentage of chunk size)
    Supports both single and multiple separators with recursive splitting.
    """
    provider = "count_words_chunker"

    def __init__(
            self,
            chunk_size: int = 420,
            chunk_overlap: int|float = 69,
            tokenizer: Tokenizer|Type[Tokenizer]|Callable[..., int] = StrSplitTokenizer,
